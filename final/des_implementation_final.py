...
# PC-1 (key initial permutation, 64 → 56 bits)
PC_1 = [
    57, 49, 41, 33, 25, 17,  9,
     1, 58, 50, 42, 34, 26, 18,
    10,  2, 59, 51, 43, 35, 27,
...
def generate_round_keys(key64: bytes) -> List[List[int]]:
    ...
    for shift in SHIFT_SCHEDULE:
        ...
        CD = C + D
        round_key = permute(CD, PC_2)  # 48 bits
...
def f_function(R_bits: List[int], round_key_bits: List[int]) -> List[int]:
    E_bits = permute(R_bits, E)  # 48 bits
    ...
    for i in range(8):
        ...
        out_bits += [(s_val >> (3 - j)) & 1 for j in range(4)]
    return permute(out_bits, P)

# ------------------ DES Encrypt/Decrypt Block ------------------

def des_encrypt_block(block8: bytes, round_keys: List[List[int]]) -> bytes:
    if len(block8) != 8:
        raise ValueError("Block size must be exactly 8 bytes")
    block_int = int.from_bytes(block8, byteorder="big")
    bits = _to_bitlist(block_int, 64)
    ...
    for rk in round_keys:
        ...
        new_L = R
        new_R = [l ^ f for l, f in zip(L, f_out)]
        L, R = new_L, new_R
    final_bits = R + L  # swap halves
    final = permute(final_bits, FP)
    final_int = _from_bitlist(final)
    return final_int.to_bytes(8, byteorder="big")

def des_decrypt_block(block8: bytes, round_keys: List[List[int]]) -> bytes:
    return des_encrypt_block(block8, list(reversed(round_keys)))
...
def test_cipher():
    ...
    for pt in plaintexts:
        ciphertext = des_encrypt_block(pt, round_keys)
        decrypted = des_decrypt_block(ciphertext, round_keys)

        print(f"Plaintext : {pt}")
        print(f"Ciphertext: {ciphertext.hex()}")
        print(f"Decrypted : {decrypted}")
        print("-" * 40)
        assert decrypted == pt, "[ERROR] Decryption failed!"
